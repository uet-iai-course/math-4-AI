...
4. Sau khi hai reviewer hoàn tất, giao `openrouter_writer` một nhiệm vụ ghi có
   ...
   - điều phối viên tạo `/tmp/math-4-AI-openrouter-smoke/`, truyền thư mục này làm `--repo-root` và truyền gốc kho làm `--api-key-root`;
   - worker chỉ gọi công cụ với đường dẫn tương đối `worker-check.txt`, tạo ra `/tmp/math-4-AI-openrouter-smoke/worker-check.txt`;
