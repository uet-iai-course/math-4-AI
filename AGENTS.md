...
- Người dùng cho phép trước việc chủ động commit và push sau mỗi thay đổi lớn đã hoàn tất các kiểm tra phù hợp, không cần xin xác nhận lại cho từng lần. Quyền này áp dụng cho nội dung bài giảng, tài liệu học tập, mã nguồn, hạ tầng và quy định của kho, bao gồm `AGENTS.md`. Thay đổi lớn là một phần việc có kết quả độc lập có thể rà soát, chẳng hạn hoàn thành dàn ý, triển khai một cụm nội dung, xử lý một nhóm lỗi hoặc cập nhật quy trình. Được yêu cầu nâng quyền thực thi khi cần ghi dữ liệu Git hoặc truy cập kho từ xa, theo cơ chế phê duyệt của môi trường.
- Mỗi mốc thay đổi lớn dùng một commit riêng, có phạm vi rõ; một bộ trang chiếu có thể có nhiều commit. Được commit và push các mốc đã kiểm tra trước khi toàn bộ bài vượt kiểm định cuối. Các mốc này không thay thế điều kiện bàn giao: bộ trang chiếu vẫn phải vượt kiểm định cuối, `index.html` được cập nhật đúng và không còn lỗi `chặn bàn giao` hoặc `nghiêm trọng` trước khi công bố hoàn thành.
- Phạm vi commit chỉ gồm các tệp thuộc phần việc vừa hoàn tất. Với bộ trang chiếu, gồm tệp `lecture-NN-<chu-de>.html`, tài sản trong `img/lec-NN/`, các tệp quy trình được Git theo dõi trong `planning/lec-NN/`, mục tương ứng trong `index.html` và những thay đổi tệp dùng chung thực sự cần thiết. Với thay đổi quy định hoặc hạ tầng, chỉ đưa vào các tệp liên quan trực tiếp. Không đưa thay đổi không liên quan, `.env`, `.env.*` hoặc bí mật vào commit; không tự buộc Git theo dõi tệp đang bị bỏ qua.
...
- Dùng `docs(agents): <mô tả ngắn>` cho cập nhật `AGENTS.md`; với thay đổi khác, dùng loại và phạm vi phản ánh đúng nội dung.
- Đẩy nhánh hiện tại lên upstream của chính nhánh đó. Trạng thái kho được kỳ vọng hiện nay là `main` theo dõi `origin/main`; phải kiểm tra lại trước khi đẩy, kể cả các commit chưa đẩy để tránh gửi thay đổi ngoài phạm vi. Không tự đổi nhánh, tạo nhánh, rebase, sửa commit đã có, xóa nhánh từ xa hoặc force-push.
