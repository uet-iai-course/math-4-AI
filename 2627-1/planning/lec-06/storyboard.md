...
|---|---|---|---|---|---|---|---|---|
| Tốc độ học thích nghi | A01 | A02 | A03 | A04–A06 | A07 | A08 | Gradient lô và momentum → giải thích $r_1,v_1^{\mathrm{RMS}},m_1^{\mathrm{Adam}},v_1^{\mathrm{Adam}}$, tính vòng hai và so sánh AdaGrad, RMSProp, Adam; truyền $g_1,g_2,\theta_0,\eta,\epsilon,\rho,\beta_1,\beta_2$ và trạng thái đầu; LLO14/CLO2,3 | A03 là ví dụ số trước phát biểu hình thức: ba thẻ fragment lần lượt hiện tổng không quên, trung bình có quên, hai mômen; hộp fragment cuối nối sang công thức. “Một mẫu số chung không phản ánh lịch sử từng tọa độ.” $0{,}40$ LT + $0{,}20$ BT. |
...
| Chuẩn hóa theo lô | C01 | C02 và SVG | C03 | C02 | C07.1 | C07.1 | Kích hoạt $Z\in\mathbb R^{m\times d}$ → tính $\mu,\sigma^2,\widehat Z,Y$, chọn thống kê suy diễn và nêu giới hạn lô nhỏ; giữ $Z,\mu,\sigma,\epsilon,\gamma,\beta$; LLO16/CLO3,4 | C02 gộp trực quan–hình thức; C03 dùng $\epsilon>0$. C07 gộp ứng dụng với bài tập theo khung ba cột. |
...
| Tích hợp quyết định | Z01 | không áp dụng: không đưa quan hệ trực quan mới | không áp dụng: dùng lại các ví dụ đã tính | Z01: bảng quyết định hình thức hóa ánh xạ tín hiệu–công cụ | Z02 | Z02 | Bốn cụm → chẩn đoán, lựa chọn công cụ, nêu phép đo và giới hạn; LLO14–16/CLO2–4 | Chu trình rút gọn nhu cầu → hình thức → kiểm tra vì đây là cụm tổng hợp, không dạy khái niệm mới. Z03 chỉ nêu ranh giới, tài liệu và chuyển tiếp, không phải bước hình thức. P+Z: $0{,}30$ LT + $0{,}20$ BT. |
...
| A03 | Làm cụ thể ba loại bộ nhớ trên cùng $g_1$ trước khi đặt công thức thuật toán. | Trực quan → AdaGrad. | Ví dụ LLO14: $r_1=(4,1)$, $v_1^{\mathrm{RMS}}=(0{,}4,0{,}1)$, $m_1^{\mathrm{Adam}}=(0{,}2,0{,}1)$, $v_1^{\mathrm{Adam}}=(0{,}004,0{,}001)$. | sửa: gắn nhãn ví dụ dẫn nhập để các trạng thái không bị đọc như công thức chưa định nghĩa; ba thẻ fragment phân biệt tổng không quên, trung bình có quên và hai mômen; $g_2$ truyền sang A08. |
...
| A06 | Khóa hai mômen, tích theo phần tử, hiệu chỉnh lệch và trạng thái checkpoint. | RMSProp → lựa chọn. | Vận dụng Adam. | sửa: dùng $g_t\odot g_t$ và nêu $\theta_t,m_t^{\mathrm{Adam}},v_t^{\mathrm{Adam}},t$. |
...
| B02 | Taylor bậc hai làm rõ Hessian biến gradient thành bước. | Nhu cầu → ví dụ giảm chấn. | Trực giác và hình thức LLO15. | sửa: định nghĩa $g_t$, $H_t$ và độ dời $p$ trên mặt trang trước khi dùng trong $q_t(p)$. |
...
| C02 | BN là tái tham số hóa theo thống kê lô, không phải bộ tối ưu. | Nhu cầu → ví dụ BN. | Chiến lược 1 LLO16. | sửa: dùng $Z$ cho kích hoạt để không tái chiếm ký hiệu Hessian $H_t$; đồng bộ SVG và lecture note. |
| C03 | Ví dụ $z=(1,3)$ kiểm được phép chuẩn hóa với $\epsilon=0{,}01$ và tách kết quả thật khỏi giới hạn lý tưởng. | BN hình thức → hạ tọa độ. | Minh chứng tính BN cho LLO16. | sửa: không đặt $\epsilon=0$ trong thuật toán; ghi $\epsilon\to0^+$ riêng. |
...
| D02 | Tiền huấn luyện dùng nhiệm vụ nguồn có nhãn để tạo $\theta_{\mathrm{pre}}$ rồi khởi tạo nhiệm vụ đích. | Nhu cầu → tiếp tục hóa. | Chiến lược 4 LLO16. | sửa: nêu điều kiện ánh xạ tham số và giới hạn chuyển giao âm. |
| D03 | Định nghĩa chuỗi mục tiêu và minh họa khởi tạo ấm. | Tiền huấn luyện → chương trình học. | Minh chứng giải thích chiến lược 5 LLO16. | sửa: nêu cùng không gian tham số; SVG đổi “mục tiêu dễ” thành “giai đoạn đầu” vì ví dụ chỉ tịnh tiến. |
| D04 | Chương trình học hiện thực hóa tiếp tục hóa bằng phân phối ví dụ thay đổi. | Tiếp tục hóa → ví dụ lịch. | Chiến lược 6 LLO16. | sửa: hiện $q_t\to q_*$ và mục tiêu tức thời. |
...
| D06 | Yêu cầu xây dựng ba phương án độc lập trên ba tình huống có dữ kiện; can thiệp và phép đo/giới hạn chỉ hiện sau bằng fragment. | Kết D → tổng hợp. | D06.1 tiền huấn luyện; D06.2 tiếp tục; D06.3 chương trình học. | sửa lời giao nhiệm vụ và bỏ hướng dẫn chấm khỏi ghi chú. |
...
| Z02 | Một tình huống buộc lựa chọn phối hợp Adam, HF–CG, chuẩn hóa theo lô và học theo chương trình; mỗi lựa chọn có phép đo kiểm chứng và một giới hạn. | Bảng quyết định → ranh giới. | Tổng hợp–lựa chọn LLO14–16/CLO2–4; phép tính đã được đo ở A08, B06 và B08, không đo lại tại đây. | thêm: đánh giá khả năng tích hợp và phân biệt vai trò. |
