...
| P01 | Mục tiêu bài học | Nêu một luận điểm trung tâm về năng lực cuối buổi (biến dữ liệu thành mô hình và dự đoán kèm bất định) cùng bốn mục tiêu quan sát được M0.1–M0.4 nhóm gọn; không lặp lưới ba thẻ của P02. |
...
| A02 | Véc-tơ và phép toán | Tính tích vô hướng $\boldsymbol\phi_1^T\mathbf w^{(0)}=11{,}5$ và chuẩn $\ell_2$ trên dữ liệu căn thứ nhất; phép tính chuẩn tách hai dòng: tổng số ngắn $1+2{,}25+1=4{,}25$ rồi suy chuẩn $\sqrt{4{,}25}$, kèm giải thích ba số là bình phương ba thành phần nằm trong ghi chú diễn giả | Goodfellow et al. (2016), §2.1–2.2 |
...
| A05 | Hạng và phụ thuộc tuyến tính | Định nghĩa không gian cột và hạng (câu đầu rút gọn, Col(A) trước rank); so sánh trực tiếp ma trận hạng 2 với ma trận hạng 1; ma trận và rank tách các dòng riêng trong mỗi thẻ; hộp kết luận rút gọn | Goodfellow et al. (2016), §2.4, §2.6 |
...
| A15 | Bài tập ứng dụng đại số tuyến tính | Một câu trên lớp về hàng–mẫu và kích thước; hai câu tự kiểm về dự đoán, phần dư và hình chiếu | Tổng hợp |
...
| B02 | Bề mặt và hai lát cắt | Trực quan bề mặt và hai lát cắt $v=2$, $u=1$; chưa tính độ dốc hoặc nêu định nghĩa | Goodfellow et al. (2016), §4.3; Boyd & Vandenberghe (2004), §A.4 |
| B03 | Lát cắt và tiếp tuyến | Tính hai độ dốc $4,5$ trên các tiếp tuyến chính xác, rồi chốt đạo hàm riêng là đạo hàm của lát cắt | Goodfellow et al. (2016), §4.3; Boyd & Vandenberghe (2004), §A.4 |
| B04 | Gradient | Gom các đạo hàm riêng thành véc-tơ; tính $\nabla h(1,2)=(4,5)^T$ và nêu hướng tăng nhanh nhất khi gradient khác không | Goodfellow et al. (2016), §4.3; Boyd & Vandenberghe (2004), §A.4 |
| B05 | Đạo hàm theo hướng | Giả sử $h$ khả vi; định nghĩa $D_{\mathbf u}h=\nabla h^T\mathbf u$ với $\|\mathbf u\|_2=1$ và tính $32/5$ | Goodfellow et al. (2016), §4.3; Boyd & Vandenberghe (2004), §A.4 |
| B06 | Vi phân và xấp xỉ tuyến tính | Dùng $\Delta\mathbf x=(0{,}1,-0{,}1)^T$ để tính $\mathrm dh=-0{,}1$ và dự đoán $6{,}9$ | Goodfellow et al. (2016), §4.3; Boyd & Vandenberghe (2004), §A.4 |
...
| B09 | Ma trận Hessian | Định nghĩa từng phần tử $\partial^2h/(\partial x_i\partial x_j)$ và tính ma trận $[[2,1],[1,2]]$ | Goodfellow et al. (2016), §4.3.1; Boyd & Vandenberghe (2004), §A.4 |
...
| B11 | Khai triển Taylor bậc hai | Nêu giả thiết đủ trơn và xấp xỉ cục bộ; ví dụ cho $6{,}91$, là đẳng thức vì $h$ bậc hai | Goodfellow et al. (2016), §4.3.1; Boyd & Vandenberghe (2004), §A.4 |
...
| B13 | Tích phân nhiều biến | Dùng $\psi$ trên miền; tính tích phân bằng $3$; hộp rút thành "Với đại lượng trên miền hai chiều, tổng tích lũy phải cộng trên toàn miền $D$."; câu nối rút thành "C05 dùng lại phép tích lũy này để biến mật độ thành xác suất của một miền."; giữ ví dụ bằng $3$ và neo hai chiều | Stewart (2016), §§15.1–15.2; Goodfellow et al. (2016), §3.3 |
...
| B19 | Bài tập tích hợp giải tích | Một câu ứng dụng giá nhà trên lớp; tự kiểm: (1) gradient, đạo hàm hướng và Taylor với $\boldsymbol\Delta=(0,1,0)^T$ tại $\mathbf x=(0,1)^T,\mathbf u=(1,0)^T$; (2) gradient của $g\circ\mathbf F$ tại $(0,1)$ bằng Jacobian; (3) tính lại hai tích phân ở B12–B13; chi tiết liên hệ C05 chuyển sang ghi chú | Tổng hợp |

### Xác suất một biến

| Mã | Tiêu đề | Hành trình | Nguồn |
|---|---|---|---|
| C01 | Phép thử, không gian mẫu và biến cố | Dùng xúc xắc đều trong không gian hữu hạn; $\Pr$ mới mang nghĩa trực giác theo kết quả đồng khả năng | Koller & Friedman (2009), Ch. 2 |
...
| D10 | Bình phương tối thiểu tổng quát | Tách mô hình nhiễu 4D khỏi ví dụ 2D $(\boldsymbol\Sigma_2,\mathbf r_\pm,J_2)$; dòng kích thước giữ cỡ thân bài; đầu ra GLS được E01/E04/E06 tiêu thụ | Goodfellow et al. (2016), §3.9, §5.5 |
...
|---|---|---|---|
| E01 | Quy trình xây dựng mô hình giá nhà | Tổng hợp ba công cụ thành quy trình; tiêu thụ đầu ra GLS của D10 qua nhánh nhiễu OLS/GLS. | Goodfellow et al. (2016), Ch. 2–5 |
| E02 | Trọng số bình phương tối thiểu | Diễn giải nghiệm bằng đơn vị tiền. | Tự tính |
| E03 | Phần dư sau khi khớp mô hình | Nêu $\mathbf X^T\mathbf r=\mathbf0$ nhưng từng phần dư có thể khác không. | Tự tính; Boyd & Vandenberghe (2004), §A.5 |
...
| E05 | Mô hình mở rộng và nguy cơ quá khớp | Dùng $(\widetilde{\boldsymbol\phi},\widetilde{\mathbf w})\in\mathbb R^4\times\mathbb R^4$; SSE huấn luyện thấp không chứng minh dự đoán ngoài mẫu tốt hơn; gọi lại A11. | Goodfellow et al. (2016), §5.2 |
| E06 | Bài tập tích hợp và chuẩn bị Bài 01 | Công thức nghiệm đóng, một bước gradient, giới hạn của SSE bằng 0, phân biệt nhiễu/phần dư; tự kiểm chọn OLS/GLS theo giả thiết nhiễu (tiêu thụ D10); tổng kết thu hồi M0.1–M0.4. | Tổng hợp |
