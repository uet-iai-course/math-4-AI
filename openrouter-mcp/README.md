...
```bash
mkdir -p /tmp/math-4-AI-openrouter-smoke
uv run openrouter-mcp-writer \
  --repo-root /tmp/math-4-AI-openrouter-smoke \
  --api-key-root .. \
  --json \
  "Chỉ tạo worker-check.txt với nội dung được yêu cầu."
```

`--api-key-root` chỉ cho tiến trình điều phối biết nơi nạp `.env`; thư mục này
không được đặt vào `MCP_REPO_ROOT`, schema công cụ, prompt, log tiến độ hoặc kết
quả worker. Nhờ đó writer có thể chỉ nhìn thấy thư mục tạm mà vẫn dùng khóa do
điều phối viên quản lý ở gốc kho.
