                    ...
                    response = await _await_with_heartbeats(
                        http.post(
                            ...
                            headers={
                                ...
                                "HTTP-Referer": "https://github.com/uet-iai-course/math-4-AI",
                                "X-OpenRouter-Title": f"Math 4 AI {role} MCP worker",
                            },
                            json={
                                "model": model,
                                "messages": messages,
...
def _parser(forced_role: str | None = None) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--api-key-root",
        help=(
            "Coordinator root containing .env; defaults to --repo-root. "
            "This path is not exposed through worker tools."
        ),
    )
    parser.add_argument(
        "--task-profile",
...
def main(forced_role: str | None = None) -> None:
    ...
    api_key = load_openrouter_api_key(args.api_key_root or args.repo_root)
    if not api_key:
