...
class ReadOnlyServerTest(unittest.TestCase):
    ...
    def test_role_specific_parser_cannot_be_overridden(self) -> None:
        ...
        self.assertIsNone(args.api_key_root)
        with self.assertRaises(SystemExit):
            _parser("reader").parse_args(["--role", "writer", "task"])

    def test_api_key_root_is_separate_from_worker_repo_root(self) -> None:
        args = _parser("writer").parse_args(
            [
                "--repo-root",
                "/tmp/scoped-worker",
                "--api-key-root",
        ...
        self.assertEqual(args.repo_root, "/tmp/scoped-worker")
        self.assertEqual(args.api_key_root, "/project/coordinator")
